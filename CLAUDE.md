...
- **Testing windows outside the app**: hand-pushed `DispatcherFrame` + `DispatcherTimer` pumps can wedge after a few windows open and close (seen while testing the snooze menu), even though the app itself is fine. For multi-window UI checks, drive the steps from inside a real `Application.Run()` loop with `await Task.Delay(...)`.
